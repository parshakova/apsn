...
def main():
    ...
    for i, batch in enumerate(batches):
        for a in range(args.n_actions):
            ...
            X[i].append([embeds, actions, questions, computed_a])
        if i >= itrs-1:
            break
    ...
    a = np.reshape(computed_a, (itrs*args.batch_size))
    a_oh = np.expand_dims(np.eye(args.n_actions)[a], -1)
    tsne_d_r = np.reshape(tsne_d, (itrs*args.batch_size, args.n_actions, -1))
    ...
    plt.scatter(x, y, c=c, s=20, cmap=discrete_cmap(N, 'jet'), alpha=0.5)
    ...
    plt.scatter(x, y, c=c, s=50,marker='x', cmap=discrete_cmap(N, 'jet'))
